...
object Util {
  ...
  def interset[E](a: java.util.Set[E], b: java.util.Set[E]): java.util.Set[E] = {
    val sizeA = a.size
    val sizeB = b.size
    val result = preSizedJavaHashSet[E](Math.min(sizeA, sizeB))
    val (smaller, larger) = if (sizeA <= sizeB) (a, b) else (b, a)
    smaller.forEach { elem => if (larger.contains(elem)) result.add(elem) }
    result
  }
