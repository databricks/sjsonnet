...
object Std {
  sealed trait ReadWriter[T]{
    def apply(t: Val, extVars: Map[String, ujson.Value], wd: Path): Either[String, T]
    def write(t: T): Val
  }
  object ReadWriter{
    implicit object StringRead extends ReadWriter[String]{
      def apply(t: Val, extVars: Map[String, ujson.Value], wd: Path) = t match{
    ...
    implicit object ValRead extends ReadWriter[Val]{
      def apply(t: Val, extVars: Map[String, ujson.Value], wd: Path) = Right(t)
      def write(t: Val) = t
    }
  ...
  case class Applyer(f: Val.Func, extVars: Map[String, ujson.Value], wd: Path){
    def apply(args: Lazy*) = {
      f.apply(args.map((None, _)), "(memory)", extVars, -1, wd, wd / "(memory)")
    }
  }
  ...
  def builtin[R: ReadWriter, T1: ReadWriter](name: String, p1: String)
                             (eval:  (Path, Map[String, ujson.Value], T1) => R): (String, Val.Func) = builtin0(name, p1){ (vs, extVars, wd) =>
  ...
  def builtin[R: ReadWriter, T1: ReadWriter, T2: ReadWriter](name: String, p1: String, p2: String)
                                             (eval:  (Path, Map[String, ujson.Value], T1, T2) => R): (String, Val.Func) = builtin0(name, p1, p2){ (vs, extVars, wd) =>
  ...
  def builtin[R: ReadWriter, T1: ReadWriter, T2: ReadWriter, T3: ReadWriter](name: String, p1: String, p2: String, p3: String)
                                                             (eval:  (Path, Map[String, ujson.Value], T1, T2, T3) => R): (String, Val.Func) = builtin0(name, p1, p2, p3){ (vs, extVars, wd) =>
  ...
  def builtin0[R: ReadWriter](name: String, params: String*)(eval: (Seq[Val], Map[String, ujson.Value], Path) => R) = {
    name -> Val.Func(
      None,
  ...
  /**
    ...
    */
  def builtinWithDefaults[R: ReadWriter](name: String, params: (String, Option[Expr])*)(eval: (Map[String, Val], Map[String, ujson.Value], Path) => R): (String, Val.Func) = {
    name -> Val.Func(
      None,
      Params(params),
