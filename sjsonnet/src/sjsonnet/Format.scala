...
//  val `type`      = P( "b" | "c" | "d" | "e" | "E" | "f" | "F" | "g" | "G" | "n" | "o" | "s" | "x" | "X" | "%" )
  ...
  val intp = new PythonInterpreter
  intp.exec("import json")
  def format(s: String, values0: ujson.Js): String = synchronized{
    val values = values0 match{
      case x: ujson.Js.Arr => ujson.Js.Arr(x.value.map{
        case ujson.Js.True => ujson.Js.Str("true")
        case ujson.Js.False => ujson.Js.Str("false")
        case x => x
      })
      case x: ujson.Js.Obj => x
      case x => x
    }
    val wrap = if (values.isInstanceOf[ujson.Js.Arr]) "tuple" else ""
    val out = new StringWriter()
    ujson.Renderer.escape(out, ujson.write(values), true)
    val cmd = ujson.write(s) + " % " + wrap + "(json.loads(" + out + "))"
    intp.eval(cmd).asString()
  }
}