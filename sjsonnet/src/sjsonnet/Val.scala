...
object Val {
  ...
  final class Obj(
    ...
    def addSuper(pos: Position, lhs: Val.Obj): Val.Obj = {
      ...
      val keysInThisChain = new java.util.HashSet[String]()
      for (s <- objs) {
        s.getValue0.keySet().forEach(k => keysInThisChain.add(k))
      }

      // Also collect excludedKeys that apply to keys in the chain
      // An excludedKey should be preserved if it's in any object's allKeyNames
      val allExcludedKeys = new java.util.HashSet[String]()
      for (s <- objs) {
        if (s.excludedKeys != null) {
          allExcludedKeys.addAll(s.excludedKeys)
        }
      }
      ...
      for (s <- objs.reverse) {
        ...
        val filteredExcludedKeys = if (s.excludedKeys != null) {
          val filtered = new java.util.HashSet[String]()
          s.excludedKeys.forEach { k =>
            // Keep the excludedKey if it's defined in any object in the chain
            if (keysInThisChain.contains(k)) filtered.add(k)
          }
    ...
    /**
     * Create a new object that removes the specified key from this object.
     *
     ...
     */
    def removeKey(pos: Position, key: String): Val.Obj = {
      val excluded = java.util.Collections.singleton(key)
      new Val.Obj(
        pos,
