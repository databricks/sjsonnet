      ...
      if (bindings == null) scope
        ...
        while (i < bindings.length) {
          ...
          newScope.bindings(base + i) = b.args match {
            ...
            case argSpec => // local function def — needs LazyFunc+closure (calls visitMethod, not visitExpr)
              if (debugStats != null) debugStats.lazyCreated += 1
      ...
      val s =
        if (bindings == null) scope
          ...
          while (i < bindings.length) {
            ...
            newScope.bindings(base + i) = b.args match {
              case null    => visitAsLazy(b.rhs)(newScope)
              case argSpec => // local function def — needs LazyFunc+closure (calls visitMethod)
                if (debugStats != null) debugStats.lazyCreated += 1
