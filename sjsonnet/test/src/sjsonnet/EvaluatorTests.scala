...
object EvaluatorTests extends TestSuite {
  def allTests(useNewEvaluator: Boolean): Tests = Tests {
    ...
    test("binaryOps") {
      val ex = assertThrows[Exception](
        eval("1 && 2", useNewEvaluator = useNewEvaluator)
      )
    ...
    test("givenDuplicateFieldsInListComprehension_expectFailure") {
      evalErr("""{ [x]: x for x in ["A", "A"]}""", useNewEvaluator = useNewEvaluator) ==>
      ...
      test - {
        val problematicStrictInheritedAssertionsSnippet =
          """local template = { assert self.flag };
            ...
            |""".stripMargin
        assert(
          evalErr(problematicStrictInheritedAssertionsSnippet, useNewEvaluator = useNewEvaluator)
  ...
  def tests: Tests =
    allTests(useNewEvaluator = false).prefix("Evaluator") ++
    allTests(useNewEvaluator = true).prefix("NewEvaluator")
}
