...
| Cache rendered bytes for repeated medium/long strings in `ByteRenderer` | Output equality on kube and `large_string_template`; renderer regression for repeated escaped strings; Native kube and `large_string_template` A/B forward+reverse; focused JVM JMH guard; full `__.test` | kept: kube Native improved in both command orders (`132.89 -> 132.28 ms` forward, `132.26 -> 130.72 ms` reverse). `large_string_template` stayed neutral-to-positive (`11.07 -> 10.82 ms` forward, `11.78 -> 11.27 ms` reverse) because the huge unique template string is above the cache cap. JVM JMH guard stayed neutral (`0.666 -> 0.666 ms/op`). |
...
| Cache quoted object-key bytes in ByteRenderer | JVM compile; Native link; output equality on kube, `large_string_template`, escaped/unicode keys, and long-key fallback; kube and `large_string_template` Native forward/reverse A/B across HashMap, direct-mapped, and capped variants | reject: default HashMap was weakly positive on kube forward but reverse was neutral-to-negative by median; direct-mapped and capped variants regressed/noised negative in reverse. Reverted rather than adding cache lookup/memory risk without stable wall-clock gain. |
